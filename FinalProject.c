...
//#include "Struct.h" //attached. Struct for instructions
...
//ask about how to count how many lines of input there are
//      while !datafile and not end of array
//ask about how to get rid of the extra ffff's on some of the output
//      &FFFF hex)
//ask if I can just shift left to change these things
...
int main(void) {
    ...
    unsigned int hex[100];//while not data file
 //read hex instructions into an array from the file
    ...
    //printf("Output of the last instruction %x", hex[sizeof(hex)-1]);
    /*
        if (!datafile) {
            printf("; open failed; program ends\n");
            exit(EXIT_FAILURE);
    ...
    for(int i =0; i<27; i++){ //step through the array of hex instructions and find the opcodes
        ...
        //printf("opcode is: %X\n", opcode);
        if(opcode == 0){  // work on masking for next fields rs rt rd
                ...
                //printf("this is the current RS: %x\n", data[i].r_form.rs);
    ...
    for(int i = 0; i<27; i++){
            ...
            if(data[i].opcode == 0){
                    ...
                    signed int currentImmediate =  data[i].i_form.immediate;
                    //printf("\tx%04x", data[i].i_form.immediate);
